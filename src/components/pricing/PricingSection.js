...
const PricingSection = () => {
  ...
  const setupPackages = [
    ...
    },
    {
      ...
      price: "$18,500",
      roi: "Pays for itself if ROAS improves by 1%",
    ...
    },
    {
      ...
      price: "$28,000",
      roi: "The 'Technical Co-founder' level",
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-7xl mx-auto mb-16">
          {setupPackages.map((plan, index) => {
            const id = plan.name.toLowerCase().replace(/ /g, '-').replace('-infrastructure', '');
            return (
              <div key={index} id={id} className={`bg-white rounded-2xl shadow-lg border-2 ${plan.popular ? 'border-blue-600 scale-105 z-10' : 'border-gray-200'} overflow-hidden hover:shadow-xl transition-all flex flex-col relative`}>
                ...
                <div className="p-8 pt-0">
                  ...
                  <button
                    onClick={() => window.dispatchEvent(new CustomEvent('openContactForm', { detail: { service: plan.name } }))}
                ...
                <div className="relative z-10 grid md:grid-cols-2 gap-12 items-center">
                    ...
                    <div className="bg-white/5 backdrop-blur-md rounded-3xl p-8 border border-white/10">
                        ...
                        <div className="space-y-6">
                            {[
                                { icon: "📊", label: "Small Business", meta: "$1M-5M revenue", rec: "Analytics Foundation ($12K)" },
                                { icon: "📈", label: "Growth Stage", meta: "$5M-20M revenue", rec: "Server-Side Infra ($18.5K)" },
                                { icon: "🚀", label: "Scale Stage", meta: "$20M-100M+ revenue", rec: "Complete Data Infra ($28K)" },
                                { icon: "💼", label: "Enterprise", meta: "Multi-brand / Complex", rec: "Custom Solution (from $25K/mo)" }
                            ].map((item, i) => (
